...
test_vid = "TTEESSTT.mp4"

# Define the window duration and step
window_duration = 4  # 5 seconds
step_duration = 4  # 5 seconds (no overlap between windows)

# Initialize move counters
cross_hands_count = 0
presenting_count = 0
play_with_hair_count = 0
...
while cap.isOpened():
    ...
    if ret:
        ...
        # Check if the current window duration has passed
        if current_time - start_time >= window_duration * 30:
            # Extract points for the current window
            window_points = getPoints(test_vid, "Test Video", output_path=None)

            # Recognize the move within the window
            window_recognizer = Recognizer(templates)
            window_result = window_recognizer.recognize(window_points)

            # Update the move counters
            if window_result[0] == "Cross Hands":
                cross_hands_count += 1
            elif window_result[0] == "Presenting":
                presenting_count += 1
            elif window_result[0] == "Playing with Hair - right hand":
                play_with_hair_count += 1

            # Print the move counts for the current window
            print("Cross Hands:", cross_hands_count, "times")
            print("Presenting:", presenting_count, "times")
            print("Playing with Hair:", play_with_hair_count, "times")
